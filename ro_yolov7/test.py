...
from ro_yolov7.utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, \
    box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, increment_path, colorstr
from ro_yolov7.utils.metrics import ap_per_class, ConfusionMatrix
from ro_yolov7.utils.plots import plot_images, output_to_target, plot_study_txt
from ro_yolov7.utils.torch_utils import select_device, time_synchronized, TracedModel
...
def test(
    ...
    if training:  # called by train.py
        ...
        # 20241223 - John's Hack
        # if opt does not exist, make it with defaults
        try:
            device = opt.device
        except:
            ...
            # we assume that we are using cuda:0
            opt.device = "0"

        # default testing to use gpu 0
    ...
    if not training:
        if device.type != "cpu":
            # John edit - this has been modified to use one channel images changing "3" to "1"
            # the shape here is (batch_size, N_channels, image_shape[0], image_shape[1])
            # assumes square images
            model(
                torch.zeros(1, 1, imgsz, imgsz)
                .to(device)

...
class WandbLogger:
    def __init__(self, opt, name, run_id, data_dict, job_type="Training"):
        ...
        if self.wandb_run:
            ...
            warn(
                f"{prefix}Install Weights & Biases for YOLOR logging with "
                f"'pip install wandb' (recommended)", stacklevel=2
            )

        # 20250218 - John log the WANDB URL to environment variable for later use
        try:
            if wandb.run:
    ...
    def download_model_artifact(self, opt):
        if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
            ...
            assert epochs_trained < total_epochs, (
